...
Для теста была добавлена переменная `size`, которой присваивалось значение количества детей у корня. `size` выводится в файл для проверки алгоритма. 
```
writeNotationToFile(size , argv[2]);
```
